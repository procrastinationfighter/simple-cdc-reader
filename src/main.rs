...
async fn query_data(session: &Session,
                    stream_id: &[u8],
                    next_starting_point: chrono::Duration,
                    prepared_query: &PreparedStatement) -> Result<i64, Box<dyn std::error::Error>> {
    // The function assumes that the program slept,
    // so that next_starting_point in ms should be smaller than
    // now - NEW_DATA_DELAY_IN_MS
    // If it's not, return the old next_starting_point value.
    let right_timestamp_ms = {
        let now = chrono::Local::now().timestamp_millis();
        let new_time = next_starting_point.num_milliseconds() + DURATION_TIME_IN_MS;

        if now - NEW_DATA_DELAY_IN_MS < next_starting_point.num_milliseconds() {
            return Ok(next_starting_point.num_milliseconds());
        }

        std::cmp::min(new_time, now - NEW_DATA_DELAY_IN_MS)
    };
    let right_timestamp = chrono::Duration::milliseconds(right_timestamp_ms);

    let mut rows_stream = session
        .execute_iter(prepared_query.clone(),
                      (stream_id.to_vec(), Timestamp(next_starting_point), Timestamp(right_timestamp)))
        .await?
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        let starting_point_ms = query_data(&session, &stream_id, starting_point, &prepared_query).await?;
